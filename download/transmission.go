...
func (td *TransmissionDownloader) AnimeByTorrents(torrent []byte, path string) (map[string]string, error) {
	...
	marshal, err := json.Marshal(transmission.Arguments)

	if err != nil {
		log.Fatalln("AnimeByTorrents json转换失败", err)
	}

	dtb := &dto.DownloadByTorrentsBean{}
	err = json.Unmarshal(marshal, &dtb.Arguments)
	...
	m["hashString"] = dtb.Arguments.TorrentAdded.HashString
	m["id"] = fmt.Sprintf("%d", dtb.Arguments.TorrentAdded.Id)
	m["name"] = dtb.Arguments.TorrentAdded.Name
	m["result"] = dtb.Result
	return m, nil
}
